...
createNewOrgUnit(orgUnit: OrgUnit): Observable<CreateApiResponse<OrgUnit>> {

    const token = this.authService.getAccessToken();

    return this.http.post<CreateApiResponse<OrgUnit>>(environment.apiUrl + '/create-org-unit', orgUnit, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          });
...
updateOrgUnit(orgUnit: OrgUnit): Observable<CreateApiResponse<OrgUnit>> {
    const token = this.authService.getAccessToken();
    return this.http.put<CreateApiResponse<OrgUnit>>(environment.apiUrl + '/update-org-unit', orgUnit, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          });
...
deleteOrgUnit(orgUnitId: number): Observable<CreateApiResponse<OrgUnit>> {
    const token = this.authService.getAccessToken();
    return this.http.delete<CreateApiResponse<OrgUnit>>(environment.apiUrl + '/delete-org-unit-by-id/' + orgUnitId, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          });
